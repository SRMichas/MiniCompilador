...
public class Parser {
	...
	private void CD(){
		...
		id = c2.getToken();
		if( id != null )
			ide.add(new Identifier(id, "-", "-", c2.getLine(),"Global"));
		c = tk;
		consume(Token.SE,"{","");
	...
	private void MD(){
		...
		if( !c.getToken().equals("}")){
			...
			if( consume(Token.ID,"main","(") && ty != null){
				ide.add(new Identifier("main", "-", "", tokens.get(idx-1).getLine(),"Global"));
			}
			consume(Token.SE, "(",")");
			consume(Token.SE,")","{");
			consume(Token.SE, "{","}");
	...
	private void VDN(){
		...
		if( ty != null && nom != null && val == null){
			...
			if( !flag )
				...
				alcance = "Local";
			ide.add(new Identifier(nom, ty, val, c2.getLine(),alcance));
		}else if( val != null){
			if( !flag )
				alcance = "Global";
			else
	...
	private void TE(){
		...
		if(c.getType() == Token.DIG){
			...
			if( algote != null && !buscar(algote))
				ide.add(new Identifier(algote, "", "", lin/*,-1*/));
		}else{
		...
		if(c.getType() == Token.DIG){
			...
			if( algote2 != null && !buscar(algote2))
				ide.add(new Identifier(algote2, "", null, lin/*,-1*/));
		}else
			error(Token.ID_DIG, "Digito/Identificador");
	...
	private void AE(){
		...
		if( nom != null){
			if( !buscar(nom)){
				ide.add(new Identifier(nom, "", "", res.getLine()/*,-1*/));
		...
		if(c.getType() == Token.DIG){
			...
			if( val1 != null )
				if( !buscar(val1))
					if( !nom.equals(val1) )
						ide.add(new Identifier(val1, "", "", res.getLine()/*,-1*/));
					else
						caca2 = true;
				else
					caca2 = true;
		...
		if(c.getType() == Token.DIG){
			...
			if( val2 != null)
				if( !buscar(val2))
					if( !nom.equals(val2) )
						ide.add(new Identifier(val2, "", "", res.getLine()/*,-1*/));
					else
						caca2 = true;
				else
					caca2 = true;
